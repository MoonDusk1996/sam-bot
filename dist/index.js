...
import { appendMessage, enqueueJob, sanitizeFolderName, generateMosaic, compressImage, } from "./utils.js";
let session = null; // Sessão única por instância
const workDir = process.env.SAM_WORKDIR; // Diretório da sessão
const allowedId = process.env.SAM_ALLOWED_ID; // Usuários permitidos
venom
    .create({
    session: "SAM",
...
function start(client) {
    client.onMessage(async (message) => {
        ...
        if (message.chatId !== allowedId)
            return; // se não for um usuario autorizado, retorna.
        // --- comandos/handlers ---
        if (message.body === "/new") {
        ...
        if (message.body === "/session") {
            if (!session ||
                ...
                return client.sendText(message.from, "⚠️ Sem sessões carregadas no momento.");
            }
            await client.sendText(message.from, `Sessão para o diretorio ${session.sessionId} ativa.`);
            return;
        }
        if (message.body === "/mosaic") {
            if (!session || message.from !== session.from) {
                return client.sendText(message.from, "⚠️ A imagem não foi armazenada em nenhum diretorio pois não existem sessões ativas no momento.\n\nCaso queria armazenar uma imagem a um diretorio use */new* primeiro.");
            }
            enqueueJob(session, async () => {
                try {
                    ...
                    await client.sendImage(message.from, mosaicPath, "mosaic.jpg", `mosaico de imagens da sessão *${session.sessionId}* `);
                }
                catch (err) {
                    console.error(err);
        ...
        if (message.body === "/exit") {
            if (!session ||
                ...
                session = null;
                return client.sendText(message.from, `Saiu da sessão ${currentSession}.`);
            }
        }
        ...
        if (session?.waitingForId &&
            ...
            try {
                ...
                await client.sendText(message.from, `Sessão para o diretorio ${sessionId} ativa.`);
            }
            catch (err) {
                console.error(err);
        ...
        if (message.isMedia ||
            ...
            if (!session || message.from !== session.from) {
                return client.sendText(message.from, "⚠️ A mídia não foi armazenada pois não há sessão ativa.\n\nUse */new* para criar uma nova sessão.");
            }
            enqueueJob(session, async () => {
                if (session)
                    try {
                        ...
                        if (message.mimetype?.startsWith("image/")) {
                            const compressedPath = path.join(session.sessionPath, `compressed_${Date.now()}.jpg`);
                            try {
                        ...
                        await client.sendText(message.from, `📄 Arquivo salvo em: ${filePath}`);
                    }
                    catch (err) {
                        console.error(err);
                        await client.sendText(message.from, `❌ Erro ao salvar ${message.type}: ${err.message || "desconhecido"}`);
                    }
            });
        }
